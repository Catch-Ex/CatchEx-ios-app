...
class WritePostViewController: UIViewController, UITextFieldDelegate, UITextViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        navigationItem.setRightBarButton(UIBarButtonItem(customView: l), animated: false)
        l.rx.tapGesture()
            .when(.recognized)
            .withUnretained(self)
            .bind { owner, _ in
                owner.delegate?.update(title: owner.titleTextField.text ?? "" ,content: owner.contentTextView.text ?? "")
                owner.present(OneButtonAlertViewController(viewModel: .init(content: "저장되었습니다!", buttonText: "확인", textColor: .appColor(.primary))), animated: false) {
